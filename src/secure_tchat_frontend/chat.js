...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const state = {
        ...
        attachments: [],
        replyingTo: null // Add reply tracking
    };
    ...
    function initializeApp() {
        ...
        if (savedUser) {
            ...
            // Message input is already set up with event listeners
            // No need for a separate setup function
    ...
    function createMessageElement(message) {
        ...
        // Create reply quote if message is replying to another message
        let replyHTML = '';
        if (message.replyTo) {
            replyHTML = `
                <div class="reply-to" onclick="scrollToMessage('${message.replyTo.messageId}')">
                    <div class="reply-line"></div>
                    <div class="reply-content">
                        <div class="reply-sender">${message.replyTo.senderName}</div>
                        <div class="reply-text">${message.replyTo.text.length > 60 ? message.replyTo.text.substring(0, 57) + '...' : message.replyTo.text}</div>
                    </div>
                </div>
            `;
        }
            ...
            <div class="message">
                ...
                <div class="message-footer">
                    ...
                    <div class="message-actions">
                        <button class="reply-btn" title="Reply" onclick="replyToMessage('${message.id}')"><i class="bi bi-reply"></i></button>
                    </div>
                </div>
            </div>
        `;
    ...
    window.replyToMessage = function(messageId) {
        ...
        const sender = message.senderId === state.currentUser.id ? 
            state.currentUser : 
            state.contacts.find(c => c.id === message.senderId);
        ...
        // Show the reply UI
        showReplyUI(state.replyingTo);
        
        // Focus the message input
        elements.messageInput.focus();
    };
    
    // Function to show the reply UI
    function showReplyUI(replyInfo) {
        // Check if the reply composer already exists
        let replyComposer = document.querySelector('.reply-composer');
        if (!replyComposer) {
            // Create the reply composer
            replyComposer = document.createElement('div');
            replyComposer.className = 'reply-composer';
            elements.messageInputContainer.insertBefore(replyComposer, elements.messageInputContainer.firstChild);
        }
        
        // Set the reply composer content
        replyComposer.innerHTML = `
            <div class="reply-composer-content">
                <div class="reply-composer-info">
                    <div class="reply-to-label">Replying to ${replyInfo.senderName}</div>
                    <div class="reply-to-preview">${replyInfo.text.length > 50 ? replyInfo.text.substring(0, 47) + '...' : replyInfo.text}</div>
                </div>
    ...
    function sendMessage() {
        ...
        if (state.replyingTo) {
            newMessage.replyTo = state.replyingTo;
    ...
    window.showAttachmentOptions = function(attachmentId, type, fileName) {
        ...
        if (type.startsWith('image/') || type.startsWith('video/')) {
            menuContent = `
                ...
                <button class="option-btn" onclick="saveAttachment('${attachmentId}', '${fileName}')"><i class="bi bi-download"></i> Save</button>
            `;
        } else {
            menuContent = `
        ...
        // Position the menu near the attachment (on the left side)
    ...
    // Function to display media in a modal
    function showMediaViewer(attachment) {
